...
export class UserService {
    ...
    async create(dto: CreateUserDto): Promise<{ user: User; message: string }> {
        ...
        const verificationCode = await this.emailVerificationService.generateVerificationCode();
    ...
    async update(id: string, dto: UpdateUserDto): Promise<{ status: true; message: string }> {
    ...
    async findAllPaginated(page: number = 1, limit: number = 10): Promise<PaginationResult<User>> {
    ...
    async updatePassword(id: string, password: string): Promise<{ status: true; message: string }> {
    ...
    async verifyEmail(email: string, verificationCode: string): Promise<{ status: true; message: string }> {
        // Verify the code
        const emailVerification = await this.emailVerificationService.verifyEmailCode(email, verificationCode);
        
        // Mark verification as used
        await this.emailVerificationService.markVerificationAsUsed(emailVerification.id);
    ...
    async resendVerificationCode(email: string): Promise<{ status: true; message: string }> {
